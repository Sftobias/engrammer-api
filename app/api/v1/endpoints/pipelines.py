...
from app.api.v1.deps import get_tenant_id
...
def invoke_pipeline(req: InvokeRequest):
    try:
    ...
    pipeline = pipeline_builder(req.tenant_id)
    output = pipeline.invoke(req.tenant_id, req.session_id or "default", req.user_message, [m.model_dump() for m in req.messages])
    return InvokeResponse(output=output)

@router.post("/pipelines/end", response_model=EndConversationResponse)
def end_conversation(req: EndConversationRequest):
    try:
        reg = PIPELINES.get(req.pipeline_id)
    except KeyError as e:
        raise HTTPException(status_code=404, detail=str(e))
    pipeline_builder = reg.factory
    pipeline = pipeline_builder(req.tenant_id)
    message = pipeline.finalizar_conversacion(req.tenant_id, req.session_id or "default")
